...
import { useAdvisor, type AdvisorConfig } from './useAdvisor'
...
function cfg(over: Partial<AdvisorConfig> = {}): AdvisorConfig {
  return {
    base,
    comfortable: 500,
    max: 2000,
...
describe('useAdvisor', () => {
  it('returns strategies with comfort + max + sprint + tapered when max > comfort', async () => {
    const { result } = renderHook(() => useAdvisor(cfg()), { wrapper: wrapper() })
  ...
  it('omits max when max == comfortable', async () => {
    const { result } = renderHook(() => useAdvisor(cfg({ comfortable: 500, max: 500 })), {
      wrapper: wrapper(),
    })
    await waitFor(() => expect(result.current.data).not.toBeNull(), { timeout: 3000 })
    const data = result.current.data!
  ...
  it('marks strategies as hitsTarget when below targetMonths', async () => {
    const { result } = renderHook(
      () => useAdvisor(cfg({ targetMonths: 240 })),
      { wrapper: wrapper() },
    )
    await waitFor(() => expect(result.current.data).not.toBeNull(), { timeout: 3000 })
    const data = result.current.data!
